...
async def list_commands(ctx):
    ...
    commands_data = {
        'General': [
            ...
            ('!checktoken <token>', 'Check if a token is valid (bot or user account)'),
        ],
        'Token Management': [
            ('!addtoken <token> [label]', 'Add a token with optional label'),
            ('!rmtoken <id|token>', 'Remove token by ID or exact token'),
...
async def checktoken(ctx, token: str = None):
    """Check if a token is valid and identify its type (Bot or User)."""
    if not token:
    ...
    token_preview = token[:10] + '...' + token[-4:] if len(token) > 14 else '****'
    
    async with ctx.typing():
        result = await token_runner.check_token_type(token)
    
    if result['success']:
        user_info = result.get('user_info', {})
        color = discord.Color.blue() if user_info.get('is_bot') else discord.Color.purple()
        embed = discord.Embed(color=color, title='Token Info', description=f'Token: {token_preview}')
        embed.add_field(name='Type', value=user_info.get('type', 'Unknown'), inline=False)
        embed.add_field(name='Username', value=user_info.get('name', 'Unknown'), inline=False)
