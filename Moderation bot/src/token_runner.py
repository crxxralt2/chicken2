...
async def check_token_type(token: str) -> Dict[str, Any]:
    """Check if a token is a bot token or user token."""
    test_client = discord.Client(intents=discord.Intents.default())
    try:
        await asyncio.wait_for(test_client.login(token), timeout=10)
        is_bot = test_client.user.bot
        user_info = {
        ...
        return {'success': True, 'msg': f'Token is valid: {user_info["type"]} account', 'user_info': user_info}
    except asyncio.TimeoutError:
        return {'success': False, 'msg': 'Token validation timed out', 'user_info': None}
    except discord.LoginFailure:
        return {'success': False, 'msg': 'Invalid token', 'user_info': None}
    except Exception as e:
        return {'success': False, 'msg': f'Error checking token: {str(e)}', 'user_info': None}
    finally:
        try:
            await test_client.close()
        except:
